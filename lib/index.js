...
const getConfig = () => {
  ...
  try {
    config = new Config(require('../.zapiergraphql'));
  } catch (e) {
...
`;
  ...
  fs.mkdirSync(`${__dirname}/../graphql/query/`, { recursive: true });
  fs.writeFileSync(`${__dirname}/../graphql/query/${filename}`, output);
...
`;
  ...
  fs.mkdirSync(`${__dirname}/../graphql/mutation/`, { recursive: true });
  fs.writeFileSync(`${__dirname}/../graphql/mutation/${filename}`, output);
...
`;
  ...
  fs.mkdirSync(`${__dirname}/../${type}/`, { recursive: true });
  fs.writeFileSync(`${__dirname}/../${type}/${filename}.js`, output);
...
`;
  ...
  fs.mkdirSync(`${__dirname}/../test/${type}/`, { recursive: true });
  fs.writeFileSync(`${__dirname}/../test/${type}/${filename}.test.js`, output);
...
const addTriggerQuery = async (query) => {
  const zapierApp = require('./index.js');
  ...
  const filepath = `${__dirname}/../triggers/${filename}`;
...
const addSearchQuery = async (query) => {
  const zapierApp = require('../../zapier/index.js');
  ...
  const filename = await createQueryFile(query);
  const filepath = `${__dirname}/../searches/${filename}`;
...
const addCreateMutation = async (mutation) => {
  ...
  const filepath = `${__dirname}/../creates/${filename}`;
...
const removeAllZapierFiles = () => {
  ...
  directories.forEach((dir) => {
    try {
      fs.rmdirSync(`${__dirname}/${dir}`, { recursive: true });
    } catch (e) {
      // Ignore errors
    }
  });
