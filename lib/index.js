...
${'  '.repeat(fieldDepth.length + 1)}}`;
  ...
  const buildGQLField = (field, fieldDepth = []) => {
    ...
    return '  '.repeat(fieldDepth.length ? fieldDepth.length : 0) + `    ${field.field}: ${value}`;
  }
...
const createSamples = (fields) => {
  ...
  for (const field of fields) {
    ...
    if (sampleFieldValues.exact && sampleFieldValues.exact[field.key]) {
      samples[field.key] = sampleFieldValues.exact[field.key];
      continue;
    }
 ...
 * @param {String} query
 *
 * @returns {Promise<{file: String, contents: String}>}   Query file contents
 */
const createQueryFile = async (query) => {
...
`;
  ...
  const file = `${process.cwd()}/graphql/query/${filename}`;
 ...
 * @param {String} mutation
 *
 * @returns {Promise<{file: String, contents: String}>}   Mutation file contents
 */
const createMutationFile = async (mutation) => {
...
`;
  ...
  const file = `${process.cwd()}/graphql/mutation/${filename}`;
  fs.writeFileSync(file, contents);
 ...
 * @param {String} operation
 *
 * @returns {Promise<{file: String, contents: String}>}   Extension file contents
 */
...
`;
  ...
  const file = `${process.cwd()}/${type}/${filename}.js`;
  fs.writeFileSync(file, contents);
...
`;
  ...
  const file = `${process.cwd()}/test/${type}/${filename}.test.js`;
  fs.writeFileSync(file, contents);
...
const addTriggerQuery = async (query) => {
  ...
  const { file } = await createQueryFile(query);
...
const addCreateMutation = async (mutation) => {
  ...
  const { file } = await createMutationFile(mutation);
  const filename = path.basename(file);
